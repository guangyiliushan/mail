...
export function InboxPage() {
  ...
  const active = MOCK_MAILS.find(m => m.id === activeId) ?? MOCK_MAILS[0];
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-[380px_1fr]">
        {/* 列表 */}
        <Card className="p-2">
          <div className="px-2 py-1 text-xs text-muted-foreground">共 {MOCK_MAILS.length} 封</div>
          ...
          <ul className="divide-y">
            {MOCK_MAILS.map((m) => (
              <motion.li
                ...
                className={`p-3 cursor-pointer ${activeId === m.id ? "bg-accent" : ""}`}
              ...
              >
                ...
                <div className="text-sm text-muted-foreground">{m.from} · {m.snippet}</div>
              </motion.li>
            ))}
          </ul>
        </Card>
